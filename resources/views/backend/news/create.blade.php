...
@section('content')
    ...
    <!-- End Page Header -->
    <form class="add-new-post" action="{{ isset($news) ? route('news.update', $news) : route('news.store') }}" method="post"
                    ...
                    <div class="card-body">
                        ...
                        <div class="row col-md-12 mb-5">
                            ...
                            <div class="col-md-7 card p-3">
                                <img class="{{ isset($news) ? 'd-block' : 'd-none' }} m-auto" id="news_image"
                                    src="{{ isset($news) ? $news->media[0]->getFullUrl() : '' }}" width="150px"
                                    height="150px" />
                        ...
                        <div class="col-md-12 col-sm-12 p-0 pl-3">
                            ...
                            <div class="mt-2">
                                <select class="form-control form-control-lg mb-3 category" name="categories[]"
                                    id="category_id" multiple>
                                    <option value="">Select Category</option>
                                    @foreach ($categories as $c)
                                        <option value="{{ $c->id }}"
                                            @isset($news)@if ($news->categories->where('id', $c->id)->first()) selected @endif @endisset>
                                            {{ $c->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-12 col-sm-12 p-0 pl-3">
                            ...
                            <input class="form-control form-control-lg mb-3" id="title" type="text" name="title"
                                value="{{ old('title') ? old('title') : (isset($news) ? $news->title : '') }}"
    ...
    @endif
                                ...
                                <select class="form-control form-control-lg mb-3" name="province_id">
                                    ...
                                    @foreach ($provinces as $province)
                                        <option value="{{ $province->id }}"
                                            @isset($news)@if ($province->id == $news->province_id) selected @endif @endisset>
                        ...
                        <div class="col-md-12 col-sm-12 p-0 pl-3">
                            <label>Author </label>
                            <input class="form-control form-control-lg mb-3" id="author" type="text" name="author"
                                value="{{ old('author') ? old('author') : (isset($news) ? $news->author : '') }}"
                                placeholder="Your News Author Name">
                        </div>
                        <br>
                        <div class="col-md-12 col-sm-12 p-0 pl-3">
                            <label>Related News</label>
                            <div>
                                <select data-placeholder="Select Related News"
                                    class="js-example-basic-multiple form-control form-control-lg mb-3" id="related_news"
                                    name="related_news[]" multiple>
                                    ...
                                    @isset($news)
                                        @foreach ($newses as $k => $new)
                                            ...
                                            @endphp
                                            <option value="{{ $new->id }}"
                                                {{ !!in_array($new->id, $nId->toArray()) ? 'selected' : '' }}>
                                                {{ $new->title }}</option>
                                        @endforeach
                                    @endisset
                        ...
                        <div class="col-md-12 col-sm-12 p-0 pl-3">
                            ...
                            <div>
                                <select data-placeholder="Select Tags"
                                    class="js-example-placeholder-multiple form-control form-control-lg mb-3" id="tags"
                                    name="tags[]" multiple>
                                    @foreach ($tags as $k => $tag)
                                        ...
                                        @endisset
                                        <option value="{{ $tag->id }}"
                                            @isset($news){{ !!in_array($tag->id, $nId->toArray()) ? 'selected' : '' }}@endisset>
                                            {{ $tag->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                    ...
                    <div class='card-body p-0'>
                        <ul class="list-group list-group-flush">
                                ...
                                <span class="d-flex">
                                    <div class="custom-control">
                                        ...
                                        <input type="checkbox" class="custom-control-input" id="customSwitch"
                                            name="status" value="1"
                                            {{ (isset($news) && $news->status == 1) || old('status') ? 'checked' : '' }}>
                                ...
                                <span class="d-flex">
                                    <div class="custom-control">
                                        ...
                                        <input type="checkbox" class="custom-control-input" id="customSwitch1"
                                            name="is_main_news" value="1"
                                            {{ (isset($news) && $news->is_main_news == 1) || old('is_main_news') ? 'checked' : '' }}>
                                ...
                                <span class="d-flex">
                                    <div class="custom-control">
                                        ...
                                        <input type="checkbox" class="custom-control-input" id="customSwitch3"
                                            name="is_photo_feature" value="1"
                                            {{ (isset($news) && $news->is_photo_feature == 1) || old('is_photo_feature') ? 'checked' : '' }}>
                            ...
                            <li class="list-group-item d-flex px-3">
                                <button type="submit" id="draft" class="btn btn-sm btn-outline-accent"
