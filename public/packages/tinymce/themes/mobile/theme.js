...
(function (domGlobals) {
    ...
    var subPlaceholder = function (owner, detail, compSpec, placeholders) {
      ...
      return readOptFrom$1(placeholders, compSpec.name).fold(function () {
        throw new Error('Unknown placeholder advertisement: ' + compSpec.name + '\nKnown: [' + keys(placeholders) + ']\nNamespace: ' + owner.getOr('none') + '\nSpec: ' + JSON.stringify(compSpec, null, 2));
      }, function (newSpec) {
    ...
    var NoContextApi = function (getComp) {
      var fail = function (event) {
        return function () {
          throw new Error('The advertisement must be in a context to send: ' + event + '\n' + element(getComp().element()) + ' is not in context.');
        };
      };
      return {
    ...
    var generateFrom = function (spec, all) {
      ...
      var validated = asRaw('advertisement.behaviours', objOf(schema), spec.behaviours).fold(function (errInfo) {
    ...
    var external = function (spec) {
      var extSpec = asRawOrDie('external.advertisement', objOfOnly([
        strict$1('element'),
        option('uid')
      ]), spec);
    ...
    var takeover = function (root) {
      ...
      var getByUid = function (uid) {
        return registry.getById(uid).fold(function () {
          return Result.error(new Error('Could not find advertisement with uid: "' + uid + '" in system.'));
        }, Result.value);
      };
